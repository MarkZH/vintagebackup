...
class Now_Mock:
    """A class to mock datetime.datetime.now() and nothing else."""

    class datetime_local:
        """The mocked version of datetime."""

        def __init__(self, now: datetime.datetime) -> None:
            """Set the time to return for the mocked now()."""
            self.timestamp = now
...
class BackupTests(TestCaseWithTemporaryFilesAndFolders):
    ...
    def test_force_copy_overrides_examine_whole_file(self) -> None:
        ...
        for method in Invocation:
            ...
            for _ in range(backup_count):
                ...
                self.assertEqual(exit_code, 0, method)
            backups = util.all_backups(self.backup_path)
            self.assertEqual(len(backups), backup_count, method)
            self.assertEqual(backups[-1], util.find_previous_backup(self.backup_path), method)
            self.assertTrue(directories_are_completely_copied(*backups), method)
            self.reset_backup_folder()
    ...
    def test_compare_contents_start_starts_on_correct_date_with_earlier_comps(self) -> None:
        """Test --compare-contents-start starts on correct date even with earlier comparisons."""
        backup_start = datetime.datetime(2026, 4, 3, 19, 26, 0)
        backup_interval = datetime.timedelta(days=1)
        compare_start = backup_start + datetime.timedelta(days=3)
        compare_interval = datetime.timedelta(days=7)
...
class MockCopy2:
    """A mocked version of shutil.copy2() that fails when mocked storage has too little space."""

    def __init__(self) -> None:
        """Save original shutil.copy2() function for later use."""
    ...
    def __call__(self, user_file: Path, new_backup_file: Path, *, follow_symlinks: bool) -> None:
        ...
        self.original_copy2(user_file, new_backup_file, follow_symlinks=follow_symlinks)
...
class DeleteBackupTests(TestCaseWithTemporaryFilesAndFolders):
    ...
    def test_delete_after_deletes_too_old_backups_before_new_backup(self) -> None:
        ...
        with (patch("lib.backup.datetime", Now_Mock(now)),
    ...
    def test_out_of_space_errors_are_system_independent(self) -> None:
        ...
        windows_enospc = 112
        self.assertEqual(
            OSError(errno.ENOSPC, "Out of space", "a", None, "b").errno,
            OSError(None, "Out of space", "a", windows_enospc, "b").errno)

    def test_backups_are_deleted_with_free_up_when_storage_space_is_too_low(self) -> None:
        ...
        old_backups = util.all_backups(self.backup_path)
        file_size = 10_000_000
        create_large_files(self.backup_path, file_size)
        new_large_files = 10
        for n in range(new_large_files):
            (self.user_path/f"sub{n}").mkdir()
        create_large_files(self.user_path, file_size)
        backups_to_delete = new_large_files
        expected_remaining_backups = old_backups[backups_to_delete:]
        mock_storage = DiskUsageMock(self.backup_path, int(1.5*file_size))
        args = argparse.parse_command_line([
            "-u", str(self.user_path),
            "-b", str(self.backup_path),
            "--free-up", str(file_size)])
        ...
        expected_backups = initial_backups - backups_to_delete + 1
        self.assertEqual(len(all_backups), expected_backups)
...
class MoveBackupsTests(TestCaseWithTemporaryFilesAndFolders):
    ...
    def test_move_since_argument_selects_correct_backups(self) -> None:
        ...
        for day in range(1, 32):
            ...
            with patch("lib.backup.datetime", Now_Mock(backup_date)):
                bak.create_new_backup(
                    self.user_path,
                    self.backup_path,
                    filter_file=None,
...
class VerificationTests(TestCaseWithTemporaryFilesAndFolders):
    ...
    def test_checksum_start_starts_checksum_on_correct_date_with_earlier_checksum(self) -> None:
        """Test that --checksum-every starts checksum on correct date with earlier checksum."""
        backup_start = datetime.datetime(2026, 4, 3, 19, 26, 0)
        backup_interval = datetime.timedelta(days=1)
        checksum_start = backup_start + datetime.timedelta(days=3)
        checksum_interval = datetime.timedelta(days=14)
    ...
    def test_verify_checksum_before_deletion(self) -> None:
        ...
        with patch("lib.backup.datetime", Now_Mock(timestamp)):
            main_no_log(["-u", str(self.user_path), "-b", str(self.backup_path), "--checksum"])
    ...
    def test_verify_checksum_before_deletion_with_no_checksum(self) -> None:
        ...
        timestamp = datetime.datetime.now() - datetime.timedelta(days=2)
        with patch("lib.backup.datetime", Now_Mock(timestamp)):
            main_no_log(["-u", str(self.user_path), "-b", str(self.backup_path)])
...
class AllBackupsTests(TestCaseWithTemporaryFilesAndFolders):
    ...
    def test_all_backups_returns_only_backups(self) -> None:
        ...
        create_user_data(self.user_path)
        backups_to_create = 7
        timestamps: list[datetime.datetime] = list(
            datetime_range(
                datetime.datetime.now(),
                datetime.timedelta(seconds=10),
                backups_to_create))
